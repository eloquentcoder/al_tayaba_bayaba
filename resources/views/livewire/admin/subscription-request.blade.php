<div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 text-white">
    <div class="flex justify-between items-center">
        <h2 class="text-3xl font-bold text-white mb-6">Subscription Requests</h2>
    </div>
                        ...
                        @if ($subscription->user)
                            <tr>
                                ...
                                <td class="py-3 px-4 text-black">
                                    {{ $subscription->user->first_name . ' ' . $subscription->user->last_name }}</td>
                                <td class="py-3 px-4 text-black">{{ $subscription->user->email }}</td>
                                ...
                                <td class="py-3 px-4 text-black">
                                    <svg wire:click="selectSubscription({{ $subscription }})"
                                        id="opensubscriptionModalButton" xmlns="http://www.w3.org/2000/svg"
    ...
    <div id="createDepositModal"
        class="{{ $selectedSubscription ? 'flex' : 'hidden' }} fixed inset-0 bg-black bg-opacity-50 justify-center items-center z-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96 text-white relative">
            ...
            <div class="flex justify-between items-center mb-4 pb-3 border-b">
                ...
                <button id="closeConfirmationModalButton" class="text-[#0f277e]" wire:click="removeSubscription()">
                    &times;
                </button>
            </div>
            <div>

                <img src="{{ $selectedSubscription->payment_proof_url ?? '' }}" alt="Faysal Bank Logo"
            ...
            <div class="my-4 space-y-1">
                <div class="text-lg text-center space-x-2 text-[#0f277e]">
                    <span>Amount:</span>
                    <span
                        id="confirmationAmount">{{ $selectedSubscription ? $selectedSubscription->amount : '0.00' }}</span>
                    USD
                </div>
            </div>
            <div class="flex gap-2">
                <button wire:click="rejectSubscription"
                ...
                <button wire:click="acceptSubscription"
